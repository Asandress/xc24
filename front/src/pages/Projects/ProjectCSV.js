...
export const ProjectCSV = () => {
  ...
  async function getData() {
    if (loading) return
    let data = await axios.get(`/api/project/csv/${prjID}`)
  ...
  async function checkFile() {
    let data = await axios.get(`/api/project/csv_check_file/${prjID}`)
...
const getExport = async () => {
  let data = await axios.get(`/api/project/get_csv/${prjID}`)
  console.log(data)
  checkFile()
}
